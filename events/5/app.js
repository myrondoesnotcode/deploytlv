/* Deploy #5 — Attendee Intelligence portal (brand-light theme) */
...
/* ── data assembly ── */
const ATTENDEES = window.D5.attendees || [];
const MATCHES = window.D5.matches || [];
const TAG_COLORS = window.D5.tagColors || {};
const CLUSTERS = (window.D5.clusterDefs || []).map(d => ({
  ...d,
  members: (d.members && d.members.length)
    ? d.members
    : ATTENDEES.filter(a => (a.cluster || []).includes(d.name)).map(a => a.name)
})).filter(c => c.members.length > 0);

const BREAKOUTS = window.D5.breakoutGroups || [];
...
/* No revenue value = the question doesn't describe this person's own venture
   (employee, fund, mentor). Show no badge rather than defaulting to Pre-Revenue. */
function RevBadge({ rev }) {
  if (!rev) return null;
  return <Badge text={revLabel(rev)} bg={revStyle(rev).background} color={revStyle(rev).color} />;
}
...
function App() {
  ...
  /* Search every field a person is described by. Name/project/tagline alone missed
     the questions people actually ask at an event — "stripe" found 0 of 3, "claude code"
     0 of 74, "raising" 0 of 7. The haystack is memoised per attendee. */
  const haystack = a => {
    ...
    <div style={{ background: BG, minHeight: "100vh", fontFamily: MONO, color: INK }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <Badge text="Deploy #05" bg={ACCENT} color={BLACK} />
          <span style={{ color: MUTED, fontSize: 11, letterSpacing: 1 }}>{ATTENDEES.length} ATTENDEES · SEPTEMBER 2026 · TLV</span>
        </div>
      </div>

      {/* TABS */}
      <div className="d2-tabs" style={{ display: "flex", padding: "0 14px", borderBottom: "1px solid " + BORDER, overflowX: "auto", background: BG }}>
        {tabs.map(([k, label]) => (
          <button key={k} onClick={() => setTab(k)} className="barlow d2-tab" style={{
      ...
      {tab === "profiles" && (
        <React.Fragment>
          ...
          {/* Cluster filter. 18 clusters need ~4000px as chips, so a phone can only ever
              reach two of them. Narrow screens get a native select (one tap, whole list);
              wider screens keep the chip row. */}
          <div className="d2-filter-select" style={{ padding: "10px 16px", borderBottom: "1px solid " + BORDER }}>
            <select value={clusterFilter || ""} onChange={e => setClusterFilter(e.target.value || null)}
              aria-label="Filter by cluster"
              className="barlow" style={{
                width: "100%", background: clusterFilter ? ACCENT : "#fff",
                border: "1px solid " + (clusterFilter ? ACCENT : BORDER),
                color: clusterFilter ? BLACK : INK, fontSize: 12, fontWeight: 700,
                letterSpacing: 1, padding: "11px 12px", textTransform: "uppercase",
                outline: "none", borderRadius: 0, fontFamily: "inherit"
              }}>
              <option value="">All clusters ({ATTENDEES.length})</option>
              {CLUSTERS.map(c => <option key={c.name} value={c.name}>{c.name} ({c.members.length})</option>)}
            </select>
          </div>
          <div className="d2-chips" style={{ display: "flex", gap: 6, padding: "10px 16px", overflowX: "auto", borderBottom: "1px solid " + BORDER }}>
              ...
              <div key={a.id} className="d2-card" style={{ padding: 16 }} onClick={() => setSelected(a)}>
                ...
                {(a.collab || []).length > 0 && (
                  <div className="barlow" style={{
                    ...
                    fontSize: 11, fontWeight: 800, letterSpacing: 1.5, color: GOLD, textTransform: "uppercase"
                  }}>{a.collab.length} {a.collab.length === 1 ? "intro" : "intros"} →</div>
                )}
              </div>
            ))}
          </div>
      ...
      {tab === "matches" && (
        <div style={{ padding: 16, maxWidth: 720 }}>
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ fontSize: 12, color: MUTED, marginTop: 4, letterSpacing: 1 }}>RANKED BY SIGNAL STRENGTH · ↩ = RETURNING DEPLOY ALUM</div>
          </div>
          {MATCHES.map(m => (
                ...
                {c.members.map(m => {
                  ...
                  return (
                    <div key={m} style={{ fontSize: 12.5, color: MUTED, marginBottom: 4, display: "flex", gap: 6 }}>
                      ...
                      <span>{m}{a && a.d1 ? <span style={{ color: FAINT, fontSize: 10 }}> · {a.d1label || "alum"}</span> : null}</span>
                    </div>
                  );
                })}
              </div>
      ...
      {tab === "breakouts" && (
        <div style={{ padding: 16 }}>
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ fontSize: 12, color: MUTED, marginTop: 4, letterSpacing: 1 }}>{BREAKOUTS.length} SUGGESTED SESSIONS · CLICK A NAME TO VIEW PROFILE · ↩ = RETURNING ALUM</div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(330px,1fr))", gap: 12 }}>
            {BREAKOUTS.map(g => (
              <div key={g.id} className="d2-flat" style={{ padding: 16, borderTop: "3px solid " + (g.color || ACCENT) }}>
                ...
                {g.anchor && <div style={{ fontSize: 11, marginBottom: 10 }}><span className="barlow" style={{ color: GOLD, fontWeight: 800, letterSpacing: 1 }}>ANCHOR </span><span style={{ color: INK }}>{g.anchor}</span></div>}
...
function ForMe({ myName, setMyName, myId, setMyId, setSelected }) {
  ...
  const pickMe = a => {
    setMyId(a.id); setMyName(a.name);
    try { localStorage.setItem("d5_me", JSON.stringify({ id: a.id, name: a.name })); } catch (e) {}
  };
            ...
            <React.Fragment>
              ...
              {peers.map(a => (
                <div key={a.id} className="d2-card" style={{ padding: "12px 16px", marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }} onClick={() => setSelected(a)}>
                  <div>
                    <div className="barlow" style={{ fontSize: 16, fontWeight: 900, marginBottom: 2 }}>{a.name}{a.d1 ? <span style={{ color: FAINT, fontSize: 10, fontWeight: 400 }}> · {a.d1label || "alum"}</span> : null}</div>
                  ...
                  <span style={{ marginLeft: 10 }}><RevBadge rev={a.revenue} /></span>
                </div>
              ))}
            </React.Fragment>
          )}
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        {a.unverifiedNotes && (
          <React.Fragment>
            ...
            <div style={{ fontSize: 11, color: FAINT, lineHeight: 1.6, fontStyle: "italic" }}>{a.unverifiedNotes}</div>
          </React.Fragment>
        )}
