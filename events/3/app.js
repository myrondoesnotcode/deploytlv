/* Deploy #3 — Attendee Intelligence portal (brand-light theme) */
...
/* ── data assembly ── */
const ATTENDEES = window.D3.attendees || [];
const MATCHES = window.D3.matches || [];
const TAG_COLORS = window.D3.tagColors || {};
const CLUSTERS = (window.D3.clusterDefs || []).map(d => ({
  ...d,
...
const BREAKOUTS = window.D3.breakoutGroups || [];
    ...
    <div style={{ background: BG, minHeight: "100vh", fontFamily: MONO, color: INK }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <Badge text="Deploy #03" bg={ACCENT} color={BLACK} />
          <span style={{ color: MUTED, fontSize: 11, letterSpacing: 1 }}>{ATTENDEES.length} ATTENDEES · JULY 2026 · TLV</span>
        </div>
      </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(290px,1fr))", gap: 12, padding: 16 }}>
            {filtered.map(a => (
              <div key={a.id} className="d2-card" style={{ padding: 16 }} onClick={() => setSelected(a)}>
                ...
                <div style={{ display: "flex", gap: 6, marginBottom: 8, flexWrap: "wrap" }}>                  {a.d1 && <Badge text={"↩ " + (a.d1label || "Alum")} border={BORDER} color={MUTED} />}
                </div>
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(20,17,8,0.55)", zIndex: 200, overflowY: "auto", padding: 20 }}
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          <Badge text={revLabel(a.revenue)} bg={revStyle(a.revenue).background} color={revStyle(a.revenue).color} />          {a.d1 && <Badge text={"↩ " + (a.d1label || "Returning alum")} border={BORDER} color={MUTED} />}
